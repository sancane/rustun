...
family_address_attribute!(
    ...
    /// # Examples
    ///```rust
    /// # use stun_rs::attributes::turn::AddtionalAddressFamily;
    /// # use stun_rs::AddressFamily;
    /// let attr = AddtionalAddressFamily::new(AddressFamily::IPv6);
...
mod tests {
    ...
    fn decode_additional_address_family_constructor() {
        let attr = AddtionalAddressFamily::new(AddressFamily::IPv4);
        assert_eq!(attr.family(), AddressFamily::IPv4);

        let attr = AddtionalAddressFamily::from(AddressFamily::IPv6);
        assert_eq!(attr.family(), AddressFamily::IPv6);
    }
    ...
    fn decode_additional_address_family_value() {
        ...
        let result = AddtionalAddressFamily::decode(ctx);
        assert_eq!(
        ...
        let ctx = AttributeDecoderContext::new(None, &dummy_msg, &buffer);
        let (attr, size) =
            AddtionalAddressFamily::decode(ctx).expect("Can not decode AddtionalAddressFamily");
        assert_eq!(size, 4);
        assert_eq!(attr.family(), AddressFamily::IPv4);
